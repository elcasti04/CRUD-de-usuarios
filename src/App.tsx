...
function App() {
	...
	const [darkMode, setDarkMode] = useState(false); // Nuevo estado para modo oscuro
	...
	useEffect(() => {
		axios
			...
			.catch(() => {
				console.warn(
					'No se pudo conectar a json-server, usando usuarios por defecto',
				);
				setUsers(defaultUsers);
			});
	}, []);
	...
	const updateUser = async (dataForm: FormData, id: number): Promise<void> => {
		...
		try {
			...
			console.warn(
				'No se pudo actualizar en API, actualizando solo en memoria',
			);
	...
	return (
		<div>
			...
			<div className="flex justify-end max-w-sm mx-auto mt-6">
				<button
			...
			<form
				...
				className="max-w-sm mx-auto mt-10 p-4 border border-gray-300 rounded bg-white dark:bg-gray-900 dark:border-gray-700"
			>
